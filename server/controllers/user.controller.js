...
export async function registerUserController(request, response) {
    ...
    } catch (error) {//500 is error status
...
export async function loginController(request, response){
    try {
        ...
        if(!email || !password){
           return response.status(400).json({
               message : "Provide eamil, password",
               error : true,
               success : false
           })
        }
...
export async function verifyForgotPasswordOtp(request, response) {
     try {
        ...
        if(!email || !otp){
            return response.status(400).json({
                message : "Provide required email field email, otp",
                error : true,
                success : false
            })
        }
        ...
        //If otp is not expired and otp is correct
...
export async function userDetails(request, response){
    try {
        ...
        //WE don' want password and token so we are subtracting it 
